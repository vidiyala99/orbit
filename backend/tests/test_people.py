...
from app.models import Event, SyncRun, User
from app.people_fixtures import FIXTURE_PEOPLE
...
def test_event_guests_aliases_people_filtered_by_event(db_session):
    client = _auth_client(db_session)
    client.post("/sync-runs", json={"source": "fixture"})
    event_id = client.get("/people").json()[0]["event_id"]

    guests = client.get(f"/events/{event_id}/guests")
    assert guests.status_code == 200
    guest_list = guests.json()
    assert len(guest_list) >= 3
    assert all(p["event_id"] == event_id for p in guest_list)
    assert all(p.get("relevance") for p in guest_list)
    assert all(p["priority"] in {"needs_you", "high", "later"} for p in guest_list)

    filtered = client.get("/people", params={"event_id": event_id}).json()
    assert [p["id"] for p in filtered] == [p["id"] for p in guest_list]

    empty = client.get("/events/00000000-0000-0000-0000-000000000000/guests").json()
    assert empty == []
...
def test_import_json_creates_sync_run_and_people(db_session):
    ...
    event = Event(user_id=user_id, title="Import Night", starts_at=datetime.now(timezone.utc))
    db_session.add(event)
    db_session.commit()

    res = client.post("/people/import", json={
        "people": [
            {"name": "Casey Lin", "role": "PM", "event_id": str(event.id), "note": "n", "dm": "d"},
    ...
    listed = client.get("/people", params={"event_id": str(event.id)}).json()
    assert len(listed) == 1
...
def test_import_csv_creates_people(db_session):
    ...
    csv_body = (
        "name,role,note,dm\n"
        "Mina Cho,Designer,Long note about the booth.,Short booth DM.\n"
    )
    res = client.post(
        "/people/import",
        content=csv_body,
...
def test_needs_follow_up_excludes_future_events_and_followed_up_people(db_session):
    ...
    db_session.add(PersonModel(user_id=user_id, name="Shuo Chen", event_id=past.id, priority="needs_you"))
    db_session.add(PersonModel(user_id=user_id, name="Already Done", event_id=past.id,
                                priority="needs_you", followed_up_at=datetime.now(timezone.utc)))
    db_session.add(PersonModel(user_id=user_id, name="Future Person", event_id=future.id, priority="needs_you"))
    db_session.add(PersonModel(user_id=user_id, name="Low Priority", event_id=past.id, priority="later"))
    db_session.commit()
...
def test_patch_person_persists_followed_up_at_and_clears_follow_up_list(db_session):
    from app.models import Person as PersonModel
    ...
    person = PersonModel(user_id=user_id, name="Shuo Chen", event_id=past.id, priority="needs_you")
    db_session.add(person)
    db_session.commit()
    db_session.refresh(person)

    res = client.get("/people/needs-follow-up")
    assert {p["name"] for p in res.json()} == {"Shuo Chen"}

    followed_up_at = datetime.now(timezone.utc).isoformat()
    patched = client.patch(f"/people/{person.id}", json={"followed_up_at": followed_up_at})
    assert patched.status_code == 200
    assert patched.json()["followed_up_at"] is not None

    fetched = client.get(f"/people/{person.id}").json()
    assert fetched["followed_up_at"] is not None
