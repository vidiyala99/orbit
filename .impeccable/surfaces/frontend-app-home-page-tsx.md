...
- Reading sizes (2026-09-14, after live use: text was too small in the room): body and the opener never below 16px on phones (job title ~15.5px, chips ~13px); desktop body ~17.6px. Desktop cards grow into spare height but Skip/Keep always stay on screen; longer profiles scroll inside the card with a bottom fade, and the phone front's text column fades above the match band rather than colliding with it.
- Motion (2026-09-14, framer-motion): badge handoff is the focal moment (drag follows finger, far/fast release browses, short drag springs back; Keep swings off right with a pink wash, next badge drops onto the clip; Skip exits left; Undo returns from its side). Real 3D flip with a slight lift. Keyboard browsing and K/S are instant. Toast slides up above the dock (never over Skip/Keep) and its Undo window pauses on hover/focus. Reduced motion = crossfades only; content visible without JS.
