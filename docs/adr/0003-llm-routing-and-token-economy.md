...
- Prompts are laid out static-first so OpenAI's automatic prefix caching can hit: instructions + output schema → the user's Focus → the event's room summary → the variable Attendee batch last.
...
Consequence: Cognee runs in its own process with its own LLM config (ADR-0002), so its extraction tokens are metered by Cognee, not by Orbit's gateway. The compounding panel covers action runs, which is where the replay savings show up. Known unknown: the reasoning-effort setting for luna/terra wasn't verified. Use the lowest effort the model accepts for fast-tier tasks.
