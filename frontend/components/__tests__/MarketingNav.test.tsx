...
describe("MarketingNav", () => {
  ...
  it("shows a quiet Open app link to /attendees when a session token is present", async () => {
    document.cookie = "sc_token=tok123; path=/";
    render(<MarketingNav active="home" />);
    fireEvent.click(screen.getByRole("button", { name: /menu/i }));
    const links = await screen.findAllByRole("link", { name: /^open app$/i });
    expect(links).toHaveLength(2);
    links.forEach((link) => expect(link).toHaveAttribute("href", "/attendees"));
