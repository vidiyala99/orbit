...
/** True until the event's known start time has passed. An event with no
 *  known start (the offline fixture) is never treated as pre-event. */
...
function talkingPointsOf(value: unknown): string[] | null {
  if (!Array.isArray(value)) return null;
  const points = value.filter((v): v is string => typeof v === "string" && v.trim().length > 0);
...
export function mapGuestToAttendee(raw: unknown): AttendeeT | null {
  ...
  return {
    ...
    talking_points: talkingPointsOf(pick(rec, "talking_points", "talkingPoints")),
  };
}
...
/** `eventIdParam` picks a specific event (the dashboard's per-event links);
 *  omitted, this falls back to the most-recently-started event this user
 *  is tracking — there is no more "the one hardcoded demo event" anymore. */
export async function loadDeskGuests(eventIdParam?: string): Promise<DeskGuests> {
  ...
  try {
    return await fetchLiveDesk(token, eventIdParam);
  } catch (err) {
...
async function fetchLiveDesk(token: string, eventIdParam?: string): Promise<DeskGuests> {
  const events = await fetchJson("/events", token);
  const eventRows = (Array.isArray(events) ? events : [])
    .map((e) => asRecord(e))
    .filter((e): e is Record<string, unknown> => e !== null);
  const target = eventIdParam
    ? eventRows.find((e) => asString(pick(e, "id")) === eventIdParam)
    : eventRows[0];
  ...
  const startsAt = asString(pick(target, "starts_at"));
  const event: EventBriefT = {
    id: eventId,
    title: asString(pick(target, "title")) ?? "Event",
    datetime: formatEventWhen(startsAt),
    starts_at: startsAt,
  };
